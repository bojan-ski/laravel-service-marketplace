<x-layouts.app :title="__('Post new project')">
    ...
    <x-form method="POST" :action="route('projects.store')" :upload="true">
        ...
        {{-- description --}}
        <x-custom.textarea-custom name='description' label='Project Description' value="{{ request('description') }}"
            placeholder='Describe your project in detail' :required="true" />

        {{-- requirements --}}
        <x-custom.textarea-custom name='requirements' label='Project Requirements' value="{{ request('requirements') }}"
            placeholder='Enter project requirements' :required="true" />
