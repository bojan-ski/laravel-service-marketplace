<x-layouts.app :title="__($project->title)">
    ...
    {{-- Edit open project form --}}
    <x-form method="PUT" :action="route('projects.update', $project)" :upload="true">

        {{-- title --}}
        <x-custom.input-custom name='title' label='Project Title' value="{{ old('title', $project->title) }}"
            placeholder='Enter project title' :required="true" />

        {{-- description --}}
        <x-custom.textarea-custom name='description' label='Project Description'
            value="{{ old('description', $project->description) }}" placeholder='Describe your project in detail'
            :required="true" />

        {{-- requirements --}}
        <x-custom.textarea-custom name='requirements' label='Project Requirements'
            value="{{ old('requirements', $project->requirements) }}" placeholder='Enter project requirements'
            :required="true" />

        {{-- budget --}}
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4"
            x-data="{ budgetType: '{{ old('budget_type',  $project->budget_type) }}' }">
            {{-- budget type --}}
            <x-custom.select-custom name='budget_type' label='Budget Type'
                value="{{ old('budget_type', $project->budget_type) }}" :required="true" :options="['fixed', 'hourly']"
                xModel='budgetType' />
            ...
            <div x-show="budgetType === 'hourly'">
                ...
                <div class="relative">
                    ...
                    <input type="number" name="hour_price" id="hour_price"
                        value="{{ old('hour_price', $project->hour_price) }}"
                        class="w-full pl-8 pr-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                        step="0.01" min="0">
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            {{-- deadline --}}
            <x-custom.input-custom type='date' name='deadline' label='Deadline'
                value="{{ old('deadline', $project->deadline) }}" min="{{ date('Y-m-d') }}" :required="true" />
